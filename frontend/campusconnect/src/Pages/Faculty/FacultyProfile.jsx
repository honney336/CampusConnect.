...
const FacultyProfile = () => {
  ...
  useEffect(() => {
    ...
    if (!token || !userData?.id || userData.role !== 'faculty') {
      navigate('/login');
      return;
    }
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const profileUpdateResponse = await fetch('http://localhost:5000/api/user/update-profile', {
        method: 'PUT',
        headers: {
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="text-center">
                <div className="mx-auto w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mb-4">
                  <FaChalkboardTeacher className="w-12 h-12 text-green-600" />
                </div>
                <h2 className="text-xl font-semibold text-gray-900">{user.username}</h2>
                <p className="text-gray-600">{user.email}</p>
                <div className="mt-4 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                  <FaChalkboardTeacher className="mr-1.5 w-3 h-3" />
                  Faculty
                </div>
              </div>
